<template>
  <div class="flex justify-between px-3 py-3.5 border-b border-gray-200 dark:border-gray-700">
    <div class="flex gap-1">
      <UButton class="capitalize" :ui="{rounded:'rounded-full'}" @click="modals.form.display=true"><UIcon class="font-bold text-xl" name="i-material-symbols-light-add"/>Create new supplier</UButton>
      <Transition>
...
<UModal :ui="{ width: `sm:max-w-6xl`, overlay: { background: 'backdrop-blur-md' } }" v-model="modals.form.display"
  ...
  <UCard :ui="{
    ...
    <AdministratorSuppliersForm v-if="modals.form.display" :data="modals.form.data" @confirm-window="(display,title)=>{modals.confirmClose.display=display,modals.confirmClose.title=title}" @new-data="table.data.unshift($event)" @doing="loading.create=$event"/>
</UCard>
</UModal>
  ...
  <ConfirmModal v-model="modals.confirm.display" :title="modals.confirm.title" :input="modals.confirm.input" @is-confirmed="isConfirmed($event)"/>
</template>
