<template>
  ...
  <UCard v-else :ui="{
    ...
    <template #header>
      <div class="flex items-center justify-between">
        <h3 class="capitalize text-base font-semibold leading-6 text-white dark:text-white">
          {{ data?data.first_name+' '+(data.middle_name?data.middle_name+' ':'')+data.last_name:'No data' }}
        </h3>
      </div>
    ...
    <AdministratorCustomersFormGroup v-if="data" :data="data" @doing="loading.create=$event" @update="data=$event"/>
</UCard>
</template>
...
async function getData(id){
  return await $fetch('/api/customers/get?'+new URLSearchParams({_id:id})).then(res=>{
    return res[0]
  })
}
onBeforeMount(async()=>{
