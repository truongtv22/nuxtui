  ...
  <div class="min-h-full flex flex-col">
    <div class="flex justify-between px-3 py-3.5 border-b border-gray-200 dark:border-gray-700">
      <div class="flex gap-1">
        <UButton class="capitalize" :ui="{ rounded: 'rounded-full' }" @click="showForm()">
          <UIcon class="font-bold text-xl" name="i-material-symbols-light-add" />Create new Customer
        </UButton>
        <Transition>
    ...
    <UModal :ui="{ width: `sm:max-w-6xl`, overlay: { background: 'backdrop-blur-md' } }" v-model="modals.form.display"
      ...
      <UCard :ui="{
        ...
        <AdministratorCustomersFormGroup v-if="modals.form.display" :data="modals.form.data"
...
const heightRow = ref(null),
  headerRef = ref(null)
const pageCount = computed(() => {
  if (tableEl.value && table.value.data.length > 0 && heightRow.value) {
    return Math.floor((basicStore.screenSize.h - tableEl.value.offsetTop - headerRef.value.parentElement.offsetHeight-20) / heightRow.value)
  }
  else {
    return 1
  }
...
onMounted(async () => {
  ...
  await $fetch('/api/customers/list').then(res => {
    if (res.length > 0) {
      table.value.data = res.sort((item1, item2) => item1.created_at < item2.created_at)
      setTimeout(() => {
        heightRow.value = Math.ceil(tableEl.value.children[0].children[0].offsetHeight) - Math.ceil(headerRef.value.parentElement.offsetHeight) + 1
      }, 1)
