<template>
  <UForm ref="form" :schema="schema" :state="state">
    <UFormGroup label="Tên nha cung cap" name="name">
      <UInput ref="names" v-model="state.name" @keyup.enter="$event.target.blur(), onSubmit()" />
    </UFormGroup>
    ...
    <UFormGroup label="Photos" name="images">
      <div
        ...
        <div v-if="state.previewImages.length == 0" @click="state.refs.file.click()"
        ...
        <div v-else class="grid grid-cols-1">
          <div class="grid grid-cols-2 sm:grid-cols-4 p-2 gap-2">
            <PreviewImage v-for="data, index in state.previewImages" :data :index :loading="data.meter.value"
              :status="data.meter.display" @remove="state.previewImages.splice(index, 1)" />
          </div>
          <UDivider />
...
function onSubmit(event: FormSubmitEvent<Schema>) {
  //emits('submiting',true)
}
async function submited() {
  const isValid = await schema.safeParse(state.value)
  const nameValid = await $fetch('/api/suppliers/get?' + new URLSearchParams({ name: state.value.name })).then(response => {
    return response.length < 1
  })
  await Promise.all([isValid, nameValid]).then(([isValid, nameValid]) => {
    if (isValid.success && nameValid) {
      const promise = async () => {
        for await (const obj of state.value.images) {
          const objIndex = state.value.images.indexOf(obj)
          state.value.previewImages[objIndex].meter.display = true
          const interval = setInterval(() => {
            if (state.value.previewImages[objIndex].meter.value < 100) {
              state.value.previewImages[objIndex].meter.value += 1
            }
            if (state.value.previewImages[objIndex].meter.value == 100) {
          ...
          await uploadFile(obj.file).then(async (res) => {
            ...
            if (res?.status == 'success') {
              ...
              state.value.previewImages[objIndex].meter.value = 100
              state.value.previewImages[objIndex].meter.display = false
              clearInterval(interval)
            }
          })
        }
    ...
    if (!isValid.success) {
      ...
      emits('status', {status:'done',error:true})
    }
  })
