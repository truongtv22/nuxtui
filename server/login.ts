...
export default async () => {
  ...
  let data = {
    phone: '84927857019',
    password:
      'bcb1bac5908e6224e4264074451c22ec21ea82fc69665036e90766ec28cbc164',
    support_ivs: true,
    client_identifier: {
  ...
  const rs = await $fetch
    .raw('https://shopee.vn/api/v4/account/login_by_password', {
      method: 'post',
      body: JSON.stringify(data),
    })
    .then(async (rs0) => {
      ...
      if (js['error'] == 0) {
        ...
        const rss = await $fetch
          .raw('https://banhang.shopee.vn/api/v2/login/?SPC_CDS=1', { headers })
          .then((rs1) => {
        ...
        const rs1 = await $fetch
          .raw('https://shopee.vn/api/v4/anti_fraud/ivs/methods', {
            headers,
            method: 'post',
            body: JSON.stringify(data),
          })
              ...
              const rs22 = await $fetch
                ...
                .then(async (rs3) => {
                  if (rs3.status < 400) {
                    ...
                    url =
                      'https://shopee.vn/api/v4/anti_fraud/ivs/link/get_status';
                    data = { r_token: rToken };
                    let stop = false;
                    ...
                    const rsxx = promise.then(async (rsx) => {
                      url =
                        'https://shopee.vn/api/v4/anti_fraud/ivs/token/verify';
                      data = {
                        method_name: 8,
                        event: 1,
                        u_token: ivsToken,
                      ...
                      rs3 = await $fetch
                        ...
                        .then(async (rs4) => {
                          if (rs4.status < 400) {
                            ...
                            url =
                              'https://shopee.vn/api/v4/account/basic/login_ivs';
                            data = {
                              is_user_login: true,
                              is_web: true,
                              ivs_flow_no: ivsFlow,
